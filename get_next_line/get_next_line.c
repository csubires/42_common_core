...
/*   Updated: 2024/08/16 11:05:50 by csubires         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char		*line;
	static char	*buffer_global;

	if (fd < 0 || BUFFER_SIZE < 1)
		return (0);
	buffer_global = ft_fill_buffer(fd, buffer_global);
	if (!buffer_global)
	{
		ft_free_ptr(&buffer_global);
		return (0);
	}
	line = ft_cut_line(buffer_global);
	buffer_global = ft_cut_remainder(buffer_global);
	if (!line)
		ft_free_ptr(&buffer_global);
	return (line);
}
